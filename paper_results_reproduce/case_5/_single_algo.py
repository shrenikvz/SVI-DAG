#!/usr/bin/env python
"""
Case 5: per-algorithm driver
=============================

Each ``run_<algo>_only.py`` calls ``run_subset(labels=[...], output_suffix=...)``
to fit one (or a related group of) algorithm(s) on every cell of the case 5
grid and write a long-form CSV (plus an aggregated JSON) with the four CPDAG
metrics (Brier, E[SHD], E[F1], AUROC).

Grid (defined in ``common.py``)
    scenarios   : ER_p50_s80                          (1)
    sample sizes: 100, 316, 1000, 3162, 10000        (5; round(10**[2, 2.5, 3, 3.5, 4]))
    replicates  : 0..NUM_REPLICATES-1                (5)
    launched algorithms: SVI-DAG + 5 baselines       (6)

A task may own a slice of that grid rather than all of it -- see the grid
subsetting block below (``CASE5_SAMPLE_SIZES`` / ``CASE5_REP_START`` /
``CASE5_REP_END``), which is how ``run_case5.sh`` gives each array task one
(algorithm, n) block.
...
Output (under ``paper_results_reproduce/case_5/``)
    case_5_results_<suffix>.csv  -- long-form rows for plotting (one per
                                    algorithm/scenario/n/replicate cell).
    case_5_results_<suffix>.json -- aggregated mean ± SE over replicates.
...
# Always force the case-local dir to sys.path[0] so its ``common.py`` /
# ``svidag_runner.py`` shadow the case_4 ones (same module names exist in
# both directories; without this, when Python auto-prepends the script dir
# the conditional ``if _p not in sys.path`` above leaves _CASE4_DIR ahead
# of _THIS_DIR and the wrong ``common`` is imported -- ImportError on
# ``SCENARIOS`` etc.).
...
# Reuse the case_4 baseline wrappers as-is.  They take
# ``(X_train, num_nodes, num_posterior_samples, seed)`` and return
# ``(A_samples, convention)``.
...
NUM_REPLICATES = int(os.environ.get("CASE5_NUM_REPLICATES", NUM_REPLICATES_DEFAULT))
NUM_POSTERIOR_SAMPLES = int(os.environ.get("CASE5_POSTERIOR_SAMPLES", "1000"))


# ---------------------------------------------------------------------------
# Grid subsetting: one Slurm array task owns one (algorithm, n) block.
#
# Every baseline whose per-step cost is linear in the sample size is split by
# n so a slow p=50, n=10^4 cell cannot take the four cheaper cells with it.
# CASE5_SAMPLE_SIZES narrows the n loop and CASE5_REP_START/REP_END narrow the
# replicate loop; ``run_subset`` folds the narrowing into the output suffix so
# concurrent tasks never write the same CSV.  All unset => the full grid, i.e.
# exactly the previous single-task behaviour.
# ---------------------------------------------------------------------------
def _selected_sample_sizes() -> List[int]:
    raw = os.environ.get("CASE5_SAMPLE_SIZES", "").strip()
    if not raw:
    ...
    if unknown:
        raise ValueError(
            f"CASE5_SAMPLE_SIZES={raw!r} requests {unknown}, which are not on "
            f"the case-5 grid {SAMPLE_SIZES}."
        )
    return [n for n in SAMPLE_SIZES if n in want]  # canonical order


SELECTED_SAMPLE_SIZES = _selected_sample_sizes()
REP_START = max(0, int(os.environ.get("CASE5_REP_START", "0")))
REP_END = min(NUM_REPLICATES, int(os.environ.get("CASE5_REP_END", str(NUM_REPLICATES))))
...
def _decorate_suffix(suffix: str) -> str:
    ...
    return "_".join(parts)
THRESHOLD = 0.5
USE_CPDAG = True                              # case_5 always uses CPDAG metrics
SVIDAG_NUM_ITERS = int(os.environ.get("SVIDAG_NUM_ITERS", "60000"))
SEED = int(os.environ.get("CASE5_SEED", "0"))
SKIP_UNIMPLEMENTED = bool(int(os.environ.get("SKIP_UNIMPLEMENTED", "1")))


# ---------------------------------------------------------------------------
# Algorithm registry (matches case_4's ALL_ALGORITHMS order)
# ---------------------------------------------------------------------------
...
def run_subset(labels: List[str], output_suffix: str) -> Tuple[Path, Path]:
    """Run the given algorithms over the full case 5 grid and write CSV + JSON."""
    ...
    print("=" * 80)
    print(f"  Case 5 (parallel job): suffix = {output_suffix!r}")
    ...
    # ── CSV is opened up-front so partial progress is durable across crashes.
    csv_path = _THIS_DIR / f"case_5_results_{output_suffix}.csv"
    json_path = _THIS_DIR / f"case_5_results_{output_suffix}.json"
    ...
    # running counter -- otherwise a task that runs only one n would seed that
    ...
    print("\n" + "=" * 80)
    print(f"  CASE 5 SUMMARY for '{output_suffix}'  (mean ± SE over "
          f"{NUM_REPLICATES} replicates; CPDAG metrics)")
    print("=" * 80)
