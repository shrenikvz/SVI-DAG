...
try:
    from cdt.data import load_dataset  # type: ignore
except Exception as _cdt_exc:
    def load_dataset(*_args, **_kwargs):  # type: ignore[no-redef]
        raise RuntimeError(
            "cdt.data.load_dataset is not available "
            f"(cdt failed to import: {_cdt_exc!r})."
        )
