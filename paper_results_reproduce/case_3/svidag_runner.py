#!/usr/bin/env python
"""
Case 3: SVIDAG runner (synthetic nonlinear-DAG benchmarks)
==========================================================

Identical body to ``paper_results_reproduce/case_2/svidag_runner.py``: the
runner is graph-/SEM-agnostic and accepts any ``(X, true_adj, node_names)``
triple, so the same code drives the linear (case_2) and nonlinear (case_3)
sweeps.  Two non-default behaviours vs the case_4 Sachs runner are kept:
  ...
  2. Clamps the SVIDAG mini-batch size to ``min(config.batch_size, n)`` so
     that runs with very small ``n`` (e.g. ``n=10`` from the case_3 grid)
...
# ``run_case3_all.sh``: the sbatch script exports a handful of ``SVIDAG_*``
...
def _apply_env_overrides(verbose: bool = False) -> None:
    ...
    # These Case-3-only values affect posterior sampling rather than a
...
def run_svidag_synthetic(
    ...
    dataset = _build_svidag_dataset(
        ...
        dataset_name=f"case3_cell{cell_index}_{scenario}",
    )
