...
# so case 3 (nonlinear) uses the same knobs.
# ---------------------------------------------------------------------------
_ENV_FLOAT = {
    "SVIDAG_LR": "lr",
    "SVIDAG_ETA_R": "eta_r",
...
def _apply_env_overrides(verbose: bool = False) -> None:
    ...
    # These Case-6-only values affect posterior sampling rather than a
...
def _train_svidag(
    ...
    the small-n end of the case_6 grid (n=10)."""
...
def _sample_A_hard_with_bias(model, r, rng, T_B, logit_bias, logit_scale,
                             z_scale):
    """Case-6 hard-DAG draw with the predeclared posterior tempering family.
