#!/usr/bin/env python
"""
Case 5: shared utilities (synthetic linear ER DAGs, CPDAG metrics)
==================================================================

Benchmarks SVI-DAG against five baselines on a synthetic linear-Gaussian DAG.
The single configuration reported in the paper for case 5 is

    * ER, p = 50, s = 80   ("ER_p50_s80")
...
``paper_results_reproduce/case_5/case_5_results_<suffix>.csv`` so each
...
class GraphScenario:
    ...
    label: str        # Pretty-printable scenario label, e.g. "ER_p50_s80".
...
# Single scenario reported in case 5 (ER, p = 50, s = 80, linear, Gaussian).
SCENARIOS: List[GraphScenario] = [
    GraphScenario(label="ER_p50_s80", p=50, s=80),
]
...
def generate_dataset(
    ...
    triple so that every cell of the case_5 grid is reproducible while
    different cells see independent draws.
    """
    seed_str = f"case5|{scenario.label}|n={num_samples}|rep={replicate}"
