#!/usr/bin/env python
"""
Case 6: SVIDAG runner (synthetic nonlinear-DAG benchmarks)
==========================================================

Derived directly from ``paper_results_reproduce/case_3/svidag_runner.py``:
the runner is graph-/SEM-agnostic and accepts any
``(X, true_adj, node_names)`` triple.  Two non-default behaviours vs the
case_4 Sachs runner are kept:

  1. Operates on a ``SyntheticDataset`` (not Sachs) -- node names are just
     ``["x0", ..., "x{p-1}"]`` and ``true_adj`` is the freshly generated
     synthetic DAG.
  2. Clamps the SVI-DAG mini-batch size to ``min(config.batch_size, n)`` so
     custom sample-size subsets cannot request a batch larger than the data.

Scenarios exposed: ``"strong_correct"``, ``"noninformative"``,
``"strong_incorrect"`` -- identical to case_3.
"""
...
# ``run_case6.sh``: the sbatch script exports a handful of ``SVIDAG_*``
...
#
# Identical block to ``case_3/svidag_runner.py`` so Case 6 uses the exact same
# SVI-DAG hyperparameter controls as its p=25, s=40 counterpart.
# ---------------------------------------------------------------------------
_ENV_FLOAT = {
    "SVIDAG_LR": "lr",
    "SVIDAG_ETA_R": "eta_r",
...
def _apply_env_overrides(verbose: bool = False) -> None:
    ...
    # These values inherited from Case 3 affect posterior sampling rather than
    # a ``config`` field; include them in the reproducibility log.
    for env_key in (
...
def _build_svidag_dataset(
    ...
    # predictive evaluation.  We are reporting only structural DAG metrics
    # in case_6, so we feed a single-row placeholder that keeps the dataclass
...
def _train_svidag(
    ...
    custom sample-size subsets smaller than the configured batch size."""
...
def _sample_A_hard_with_bias(model, r, rng, T_B, logit_bias):
    """Case-6 hard-DAG draw with one global posterior edge-logit shift."""
...
def _posterior_logit_bias(dataset_size: int) -> float:
    """Predeclared log-sample-size calibration used only by Case 6."""
...
def run_svidag_synthetic(
    ...
    dataset = _build_svidag_dataset(
        ...
        dataset_name=f"case6_cell{cell_index}_{scenario}",
    )
