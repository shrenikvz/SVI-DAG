#!/usr/bin/env python
"""
Case 6: per-algorithm driver (nonlinear synthetic DAGs, DAG metrics)
=====================================================================

Each ``run_<algo>_only.py`` calls ``run_subset(labels=[...], output_suffix=...)``
to fit one (or a related group of) algorithm(s) on every cell of the case 6
...
subsetting block below (``CASE6_SAMPLE_SIZES`` / ``CASE6_REP_START`` /
``CASE6_REP_END``), which is how ``run_case6.sh`` gives each array task one
(algorithm, n) block.
...
Output (under ``paper_results_reproduce/case_6/``)
    case_6_results_<suffix>.csv  -- long-form rows for plotting (one per
                                    algorithm/scenario/n/replicate cell).
    case_6_results_<suffix>.json -- aggregated mean ± SE over replicates.
...
NUM_REPLICATES = int(os.environ.get("CASE6_NUM_REPLICATES", NUM_REPLICATES_DEFAULT))
NUM_POSTERIOR_SAMPLES = int(os.environ.get("CASE6_POSTERIOR_SAMPLES", "1000"))
...
# nonlinear DiBS, nonlinear ProDAG) is substantially more expensive at
# n = 10^4 than on the old 10..1000 grid, so one task per algorithm no longer
# fits in any walltime. CASE6_SAMPLE_SIZES narrows the n loop and CASE6_REP_START/
...
def _selected_sample_sizes() -> List[int]:
    raw = os.environ.get("CASE6_SAMPLE_SIZES", "").strip()
    if not raw:
    ...
    if unknown:
        raise ValueError(
            f"CASE6_SAMPLE_SIZES={raw!r} requests {unknown}, which are not on "
            f"the case-6 grid {SAMPLE_SIZES}."
        )
    return [n for n in SAMPLE_SIZES if n in want]  # canonical order


SELECTED_SAMPLE_SIZES = _selected_sample_sizes()
REP_START = max(0, int(os.environ.get("CASE6_REP_START", "0")))
REP_END = min(NUM_REPLICATES, int(os.environ.get("CASE6_REP_END", str(NUM_REPLICATES))))
...
def _decorate_suffix(suffix: str) -> str:
    ...
    return "_".join(parts)
THRESHOLD = 0.5
USE_CPDAG = False                             # case_6 reports DAG metrics
SVIDAG_NUM_ITERS = int(os.environ.get("SVIDAG_NUM_ITERS", "60000"))
SEED = int(os.environ.get("CASE6_SEED", "0"))
...
#
# case_6's SEM is *nonlinear* (1-hidden-layer MLP, 10 ReLU units), so every
...
def run_subset(labels: List[str], output_suffix: str) -> Tuple[Path, Path]:
    """Run the given algorithms over the full case 6 grid and write CSV + JSON."""
    ...
    print("=" * 80)
    print(f"  Case 6 (parallel job): suffix = {output_suffix!r}")
    ...
    print(f"    threshold = {THRESHOLD}   use_cpdag = {USE_CPDAG}  "
          f"(case_6 -> DAG metrics)")
    print(f"    svidag_num_iters = {SVIDAG_NUM_ITERS}   seed = {SEED}")
    print("=" * 80)

    csv_path = _THIS_DIR / f"case_6_results_{output_suffix}.csv"
    json_path = _THIS_DIR / f"case_6_results_{output_suffix}.json"
    ...
    # running counter -- otherwise a task that runs only n=10^4 would seed that
    ...
    print("\n" + "=" * 80)
    print(f"  CASE 6 SUMMARY for '{output_suffix}'  (mean ± SE over "
