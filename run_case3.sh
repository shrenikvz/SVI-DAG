...
# ===========================================================================
# Case 3 -- SVI-DAG vs 5 baselines on synthetic nonlinear data (ER, p=25, s=40),
# DAG-level metrics, 7 sample sizes (n = 10^1..10^4 in half-decades)
# x 5 replicates.
#
#   sbatch run_case3.sh                     # the whole grid (210 tasks)
#   sbatch --array=0-34 run_case3.sh        # SVI-DAG, every n, every replicate
#   sbatch --array=135-139 run_case3.sh     # DDS at n=10^4 only
#
...
# mode here and DiBS runs the joint nonlinear model whose likelihood is
# full-batch, so its cost is linear in n. One task covering all seven sizes is
# a poor unit of work, and a timeout in the largest cell would take the cheap
# cells with it. Splitting per-n bounds that blast radius. ProDAG (tasks 35-69)
# is by far the most expensive row at this size (~62 min for the worst single
# replicate) -- watch it.
#
...
#   CASE3_SAMPLE_SIZES=10000 CASE3_REP_START=0 CASE3_REP_END=1 \
#       sbatch --array=0 run_case3.sh
...
#   paper_results_reproduce/case_3/case_3_results_<algo>_n<N>.csv
#   paper_results_reproduce/case_3/case_3_results_<algo>_n<N>.json
# then regenerates the figure from every slice that exists so far:
#   paper_results_reproduce/case_3/ER_p25_s40_metrics.{pdf,png}
...
#   python paper_results_reproduce/plot_cases.py --cases 3
# ===========================================================================

ALGOS=(svidag prodag bayesdag dds dibs bcd)
# Must cover every entry of case_3/common.py SAMPLE_SIZES (order here is free --
...
: "${CASE3_SAMPLE_SIZES:=$NSAMP}"
: "${CASE3_REP_START:=$(( CHUNK * REPS_PER_TASK ))}"
: "${CASE3_REP_END:=$(( CHUNK * REPS_PER_TASK + REPS_PER_TASK ))}"
export CASE3_SAMPLE_SIZES CASE3_REP_START CASE3_REP_END
echo "=== case 3 | array task $T -> $ALGO @ n=$CASE3_SAMPLE_SIZES "\
"reps [$CASE3_REP_START,$CASE3_REP_END) | $(hostname) ==="
...
#
#     SVIDAG_ENV_ACTIVATE=/path/to/env/bin/activate sbatch run_case3.sh
#
...
if [ -z "$SVIDAG_ENV_ACTIVATE" ]; then
    ...
    echo "       which Python environment to use. Edit run_case3.sh or export" >&2
...
#
# profiles/case3.env holds the exact settings required to reproduce case 3.
# The same file is sourced by ./run_local.sh, so a Slurm run and a local run of
# case 3 use identical hyperparameters. Change a value there, not here --
...
srun python -u "paper_results_reproduce/case_3/run_${ALGO}_only.py"
STATUS=$?

# Refresh the figure from every CSV present so far. Idempotent.
python -u paper_results_reproduce/plot_cases.py --cases 3 || true
exit $STATUS
