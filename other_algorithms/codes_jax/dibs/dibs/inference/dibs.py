...
class DiBS:
    ...
    def __init__(self, *,
        ...
        self.grad_mc_chunk_size = grad_mc_chunk_size
        self.verbose = verbose

    def _mc_map(self, fn, xs):
        """
        Apply ``fn`` across the leading Monte-Carlo-sample axis of ``xs``.

        Mathematically identical to ``vmap(fn, 0, 0)(xs)`` -- same samples,
        same order, same results -- but when ``grad_mc_chunk_size`` is set the
        samples are evaluated ``grad_mc_chunk_size`` at a time instead of all
        ``n_grad_mc_samples`` at once, so only that many per-sample
        intermediates are ever live.

        This matters for the nonlinear likelihood, whose per-sample working set
        is the ``[d, n_observations, d]`` parent-masked data tensor. The
        estimators below are themselves vmapped over particles, so the default
        (unchunked) path holds ``n_particles * n_grad_mc_samples`` copies of it
        simultaneously: at d=25, n=10^4, 20 particles and 128 MC samples that
        is 20*128*25*10^4*25*4 B = 60 GiB, which OOMs a 40 GB GPU. Chunking is
        the only lever here that does not alter the estimator.

        ``None`` (the default) keeps the original single-shot ``vmap``.
        """
        chunk = self.grad_mc_chunk_size
        n = xs.shape[0]
        if not chunk or chunk >= n:
            return vmap(fn, 0, 0)(xs)
        if n % chunk:
            raise ValueError(
                f'grad_mc_chunk_size={chunk} must divide n_grad_mc_samples={n}')
        xs_chunked = xs.reshape((n // chunk, chunk) + xs.shape[1:])
        out = lax.map(vmap(fn, 0, 0), xs_chunked)
        return tree_map(lambda a: a.reshape((n,) + a.shape[2:]), out)
    ...
    def eltwise_log_joint_prob(self, gs, single_theta, rng):
        ...
        return self._mc_map(
            lambda g: self.log_joint_prob(g, single_theta, self.x, self.interv_mask, rng), gs)
    ...
    def grad_z_likelihood_gumbel(self, single_z, single_theta, single_sf_baseline, t, subk):
        ...
        logprobs_numerator = self._mc_map(
            lambda e: self.log_joint_prob_soft(single_z, single_theta, e, t, subk_), eps)
        logprobs_denominator = logprobs_numerator
        ...
        grad_z = self._mc_map(
            lambda e: grad(self.log_joint_prob_soft, 0)(single_z, single_theta, e, t, subk_), eps)
    ...
    def grad_theta_likelihood(self, single_z, single_theta, t, subk):
        ...
        grad_theta = self._mc_map(
            lambda g: grad_theta_log_joint_prob(g, single_theta, self.x, self.interv_mask, subk_),
            g_samples)
