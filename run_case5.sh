...
# ===========================================================================
# Case 5 -- SVI-DAG vs 5 baselines on synthetic linear-Gaussian data (ER, p=50, s=80),
# CPDAG-level metrics, 5 sample sizes (n = 10^2..10^4 in half-decades)
# x 5 replicates.
#
#   sbatch run_case5.sh                    # the whole grid (30 tasks)
#   sbatch --array=0-4 run_case5.sh        # SVI-DAG, every n
#   sbatch --array=19 run_case5.sh         # DDS at n=10^4 only
#
...
#   CASE5_SAMPLE_SIZES=10000 CASE5_REP_START=0 CASE5_REP_END=1 \
#       sbatch --array=0 run_case5.sh
...
#   paper_results_reproduce/case_5/case_5_results_<algo>_n<N>.csv
#   paper_results_reproduce/case_5/case_5_results_<algo>_n<N>.json
# then regenerates the figure from every slice that exists so far:
#   paper_results_reproduce/case_5/ER_p50_s80_metrics.{pdf,png}
...
#   python paper_results_reproduce/plot_cases.py --cases 5
# ===========================================================================
...
: "${CASE5_SAMPLE_SIZES:=$NSAMP}"
: "${CASE5_REP_START:=$(( CHUNK * REPS_PER_TASK ))}"
: "${CASE5_REP_END:=$(( CHUNK * REPS_PER_TASK + REPS_PER_TASK ))}"
export CASE5_SAMPLE_SIZES CASE5_REP_START CASE5_REP_END
echo "=== case 5 | array task $T -> $ALGO @ n=$CASE5_SAMPLE_SIZES "\
"reps [$CASE5_REP_START,$CASE5_REP_END) | $(hostname) ==="
...
#
#     SVIDAG_ENV_ACTIVATE=/path/to/env/bin/activate sbatch run_case5.sh
#
...
if [ -z "$SVIDAG_ENV_ACTIVATE" ]; then
    ...
    echo "       which Python environment to use. Edit run_case5.sh or export" >&2
...
#
# profiles/case5.env holds the exact settings required to reproduce case 5.
# The same file is sourced by ./run_local.sh, so a Slurm run and a local run of
# case 5 use identical hyperparameters. Change a value there, not here --
...
srun python -u "paper_results_reproduce/case_5/run_${ALGO}_only.py"
STATUS=$?

# Refresh the figure from every CSV present so far. Idempotent.
python -u paper_results_reproduce/plot_cases.py --cases 5 || true
exit $STATUS
