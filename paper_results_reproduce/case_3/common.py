#!/usr/bin/env python
"""
Case 3: shared utilities (synthetic nonlinear ER DAGs, DAG metrics)
====================================================================

Benchmarks SVIDAG against 6 baselines on synthetic *nonlinear* DAGs.  The
only configuration reported here is

    * ER, p = 25, s = 40    ("ER_p25_s40")  with a paper-spec nonlinear SEM
...
``paper_results_reproduce/case_3/case_3_results_<suffix>.csv`` so each
...
# Single nonlinear scenario reported in case 3 (ER, p = 25, s = 40,
# nonlinear MLP SEM, N(0, 1) Gaussian noise).
SCENARIOS: List[GraphScenario] = [
    GraphScenario(label="ER_p25_s40", p=25, s=40, sem_type="nonlinear"),
]
...
def generate_dataset(
    ...
    triple so every cell of the case_3 grid is reproducible while different
    cells see independent draws.  We deliberately use a different seed prefix
    ("case3|...") than case_2 to avoid accidental seed collisions across
    cases.
    """
    seed_str = f"case3|{scenario.label}|n={num_samples}|rep={replicate}"
...
def evaluate_samples(
    ...
    """
    Compute Brier / E[SHD] / E[F1] / AUROC at **DAG level** (case 3 reports
