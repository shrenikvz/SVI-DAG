#!/usr/bin/env python
"""
Case 2: per-algorithm driver
=============================

Each ``run_<algo>_only.py`` calls ``run_subset(labels=[...], output_suffix=...)``
to fit one (or a related group of) algorithm(s) on every cell of the case 2
...
Grid (defined in ``common.py``)
    scenarios   : ER_p20_s40, ER_p100_s200          (2)
...
subsetting block below (``CASE2_SAMPLE_SIZES`` / ``CASE2_REP_START`` /
``CASE2_REP_END``), which is how ``run_case2.sh`` gives each array task one
(algorithm, n) block.
...
Output (under ``paper_results_reproduce/case_2/``)
    case_2_results_<suffix>.csv  -- long-form rows for plotting (one per
                                    algorithm/scenario/n/replicate cell).
    case_2_results_<suffix>.json -- aggregated mean ± SE over replicates.
...
NUM_REPLICATES = int(os.environ.get("CASE2_NUM_REPLICATES", NUM_REPLICATES_DEFAULT))
NUM_POSTERIOR_SAMPLES = int(os.environ.get("CASE2_POSTERIOR_SAMPLES", "1000"))
...
# nonlinear DiBS) costs 10-100x more at n = 10^4 / 10^5 than it did on the old
# 10..1000 grid, so one task per algorithm no longer fits in any walltime.
# CASE2_SAMPLE_SIZES narrows the n loop and CASE2_REP_START/REP_END narrow the
...
def _selected_sample_sizes() -> List[int]:
    raw = os.environ.get("CASE2_SAMPLE_SIZES", "").strip()
    if not raw:
    ...
    if unknown:
        raise ValueError(
            f"CASE2_SAMPLE_SIZES={raw!r} requests {unknown}, which are not on "
            f"the case-2 grid {SAMPLE_SIZES}."
        )
    return [n for n in SAMPLE_SIZES if n in want]  # canonical order


SELECTED_SAMPLE_SIZES = _selected_sample_sizes()
REP_START = max(0, int(os.environ.get("CASE2_REP_START", "0")))
REP_END = min(NUM_REPLICATES, int(os.environ.get("CASE2_REP_END", str(NUM_REPLICATES))))
...
def _decorate_suffix(suffix: str) -> str:
    ...
    return "_".join(parts)
THRESHOLD = 0.5
USE_CPDAG = True                              # case_2 always uses CPDAG metrics
SVIDAG_NUM_ITERS = int(os.environ.get("SVIDAG_NUM_ITERS", "60000"))
SEED = int(os.environ.get("CASE2_SEED", "0"))
...
def run_subset(labels: List[str], output_suffix: str) -> Tuple[Path, Path]:
    """Run the given algorithms over the full case 2 grid and write CSV + JSON."""
    ...
    print("=" * 80)
    print(f"  Case 2 (parallel job): suffix = {output_suffix!r}")
    ...
    csv_path = _THIS_DIR / f"case_2_results_{output_suffix}.csv"
    json_path = _THIS_DIR / f"case_2_results_{output_suffix}.json"
    ...
    print("\n" + "=" * 80)
    print(f"  CASE 2 SUMMARY for '{output_suffix}'  (mean ± SE over "
