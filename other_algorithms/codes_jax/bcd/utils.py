...
import numpy as onp
try:
    import cdt  # noqa: F401
except Exception as _cdt_exc:  # torch / cdt may not be installed in this env
    cdt = None  # type: ignore
    _CDT_IMPORT_ERROR = _cdt_exc
else:
    _CDT_IMPORT_ERROR = None
import time
...
try:
    from cdt.metrics import SHD_CPDAG  # type: ignore
except Exception:
    def SHD_CPDAG(*_args, **_kwargs):  # type: ignore[no-redef]
        raise RuntimeError(
            "cdt.metrics.SHD_CPDAG is not available (cdt failed to import: "
            f"{_CDT_IMPORT_ERROR!r}). Install cdt + torch to use SHD_CPDAG."
        )
...
def make_to_W(dim: int,) -> Callable[[jnp.ndarray], jnp.ndarray]:
    ...
    def to_W(w_params: jnp.ndarray) -> jnp.ndarray:
        ...
        # jax.ops.index_update was removed; use the functional ``.at[].set()``.
        tmp = out.at[upper_idx].set(w_params[: w_param_dim // 2])
        tmp = tmp.at[lower_idx].set(w_params[w_param_dim // 2 :])
        return tmp
...
def lower(theta: Tensor, dim: int) -> Tensor:
    ...
    out = np.zeros((dim, dim))
    # jax.ops.index_update removed in modern JAX -> functional ``.at[].set()``.
    out = out.at[np.triu_indices(dim, 1)].set(theta).T
    return out
...
def upper(theta: Tensor, dim: int) -> Tensor:
    ...
    out = np.zeros((dim, dim))
    out = out.at[np.tril_indices(dim, -1)].set(theta).T
    return out
